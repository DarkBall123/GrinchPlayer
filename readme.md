...
AI-режим расшифровывает отдельный аудиовход и предлагает пять готовых звуков только с открытой страницы. Каждая страница считается отдельным персонажем: при переключении страницы контекст разговора сбрасывается, а звуки других персонажей не индексируются и не попадают в shortlist. Для Windows настройте VB-Cable или Voicemeeter так, чтобы речь собеседника приходила в отдельный recording endpoint, а звуки GrinchPlayer уходили через другой `audiooutput`.
